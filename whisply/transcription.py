...
class TranscriptionHandler:
    ...
    def transcribe_with_whisperx(self, filepath: Path) -> dict:
        ...
        def whisperx_task(task: str = "transcribe", language=None):
            ...
            model = whisperx.load_model(
                ...
                asr_options={"hotwords": None, "multilingual": False},
            )
            audio = whisperx.load_audio(str(filepath), sr=16000)
            result = model.transcribe(
                audio, batch_size=16 if self.device == "cuda:0" else 8, task=task
            )

            model_a, metadata = whisperx.load_align_model(
                device=device, language_code=language
            )
        ...
        transcription_task = partial(
            whisperx_task, task="transcribe", language=self.file_language
        )
        ...
        try:
            pipe = pipeline(
                ...
                model_kwargs=(
                    ...
                    else {"attn_implementation": "eager"}
                ),
            )
    ...
    def transcribe_with_faster_whisper(
        ...
        # Start and time transcription
        logging.info(
            f"👨‍💻 Transcription started with 🏃‍♀️‍➡️ faster-whisper for {filepath.name}"
        )
        t_start = time.time()
    ...
    def detect_language(self, filepath, audio_array) -> str:
        ...
        def run_language_detection():
            if self.device == "whispercpp":
                from pywhispercpp.model import Model

                model = Model(self.model)
                # Note: pywhispercpp handles language detection automatically
                # Setting language to 'auto' will return the detected language
                segments = model.transcribe(str(filepath), language="auto")
                # Get first segment's language (all will be the same)
                lang = next(segments).language
                return lang, 1.0  # Score not provided by whisper.cpp
            else:
                lang_detection_model = WhisperModel(
                    models.set_supported_model(
                        self.model_provided, implementation="faster-whisper"
                    ),
                    device="cpu" if self.device in ["mps", "cpu"] else "cuda",
                    compute_type="int8" if self.device in ["mps", "cpu"] else "float16",
                )
    ...
    def process_files(self, files) -> None:
        ...
        for idx, filepath in enumerate(self.filepaths):
            ...
            filepath, audio_array = little_helper.check_file_format(
                filepath=filepath, del_originals=self.del_originals
            )
            ...
            if self.device == "whispercpp":
                self.model = models.set_supported_model(
                    self.model_provided, implementation="whispercpp"
                )
            ...
            if self.device == "mps":
                ...
                print(
                    f'→ Using {self.device.upper()} and 🚅 Insanely-Fast-Whisper with model "{self.model}"'
                )
                ...
                if self.annotate or self.subtitle:
                    self.model = models.set_supported_model(
                        self.model_provided, implementation="whisperx"
                    )
                    print(
                        f'→ Using {self.device.upper()} and whisper🆇  with model "{self.model}"'
                    )
                    ...
                    print(
                        f'→ Using {self.device.upper()} and 🏃‍♀️‍➡️ Faster-Whisper with model "{self.model}"'
                    )
            ...
            ).save_results(result=result, export_formats=self.export_formats)
