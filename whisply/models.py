...
WHISPER_MODELS = {
    ...
    "large-v3-turbo": {
        "faster-whisper": "deepdml/faster-whisper-large-v3-turbo-ct2",
        "insane-whisper": "openai/whisper-large-v3-turbo",
        "whisperx": "deepdml/faster-whisper-large-v3-turbo-ct2",  # Now points to the correct model
        "whispercpp": "./models/ggml-large-v3-turbo-q5_0.bin",
    },
}
...
def set_supported_model(model: str, implementation: str) -> str:
    if not is_model_supported(model, implementation):
        print(
            f'→ Model "{model}" is not available for this task / implementation → Using default model "large-v2".'
        )
